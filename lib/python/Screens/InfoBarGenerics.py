		...
		self["EPGActions"] = HelpableActionMap(self, "InfobarEPGActions",
			{
				...
				"showEventInfoPlugin": self.showEventInfoPlugins,
				"showInfobarOrEpgWhenInfobarAlreadyVisible": self.showEventInfoWhenNotVisible,
			})
		...
		self["SeekActions"] = InfoBarSeekActionMap(self, actionmap,
			{
				...
				"seekFwdManual": self.seekFwdManual,
				"seekBack": self.seekBack, 
				"seekBackManual": self.seekBackManual
			}, prio=-1)
...
#				"extensions": (self.showExtensionSelection, _("view extensions...")),
				"extensions": (self.showExtensionSelectionWithConfirm, _("view extensions...")),		# [iq]
...
# [iq
	def showExtensionSelectionWithConfirm(self):
		import crypt, pwd
		pw1 = pwd.getpwnam("root")[1]
		pw2 = crypt.crypt("", pw1)
		if pw1 == pw2:
			self.showExtensionSelection()
		else:
			from Screens.PasswordBox import PasswordBox
			self.session.openWithCallback(self.extensionWithConfirmCallback, PasswordBox)

	def extensionWithConfirmCallback(self, answer):
		if answer:
			self.showExtensionSelection()
# iq]
...
class InfoBarRedButton:
	def __init__(self):
		self["RedButtonActions"] = HelpableActionMap(self, "InfobarRedButtonActions",
			{
				"activateRedButton": self.activateRedButton,
			})
