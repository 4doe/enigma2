...
#		self["introduction"] = Label(_("Please choose the default channel list\nyou want to install."))
	...
	def createSetup(self):
		...
		if os.path.exists("/etc/.def_inst/install_motorized"):
			chlist_name = open("/etc/.def_inst/install_motorized", "r").readlines()[0].split("motorized-")[1].split("_")[0]
		...
		if os.path.exists("/etc/.def_inst/install_fixed"):
			chlist_name = open("/etc/.def_inst/install_fixed", "r").readlines()[0].split("motorized-")[1].split("_")[0]
	...
	def installConfirmed(self, ret):
		if ret:
			self.session.openWithCallback(self.installFinished, DefaultChannelListInstall, self.type, self.installedChLists)
...
class DefaultChannelListInstall(Screen):
	...
	def __init__(self, session, type, removelists=[]):
		Screen.__init__(self, session)
		...
		if self.type == DefaultChannelLists.MOTORIZED:
			text = _("Installing motorized channel list, please wait.")
		elif self.type == DefaultChannelLists.FIXED:
	...
	def doInstall(self):
		...
		if self.removeLists != []:
			for ch_list in self.removeLists:
				self.ret = os.system("opkg remove %s" % ch_list)
				if self.ret != 0:
					self.close(self.ret)

		if self.type == DefaultChannelLists.MOTORIZED:
			self.ret = os.system("/etc/.def_inst/install_motorized")
		elif self.type == DefaultChannelLists.FIXED:
			self.ret = os.system("/etc/.def_inst/install_fixed")
