...
# ]
		...
		if self.menu == 0:
			...
			self.list.append(("install-extensions", _("Manage extensions"), _("\nManage extensions or plugins for your Settop Box" ) + self.oktext, None))
# NOTE : Software update problem. so it remove software update menu.
#			self.list.append(("software-update", _("Software update"), _("\nOnline update of your Settop Box software." ) + self.oktext, None))
			self.list.append(("software-restore", _("Software restore"), _("\nRestore your Settop Box with a new firmware." ) + self.oktext, None))
			self.list.append(("system-backup", _("Backup system settings"), _("\nBackup your Settop Box settings." ) + self.oktext + "\n\n" + self.infotext, None))
			self.list.append(("system-restore",_("Restore system settings"), _("\nRestore your Settop Box settings." ) + self.oktext, None))
	...
	def checkTraficLight(self):
		...
		if default:
			...
			message += _("Do you want to update your Settop Box?")+"\n"+_("After pressing OK, please wait!")
...
class PluginManager(Screen, DreamInfoHandler):
	...
	def runExecuteFinished(self):
		...
		if plugins.restartRequired or self.restartRequired:
			self.session.openWithCallback(self.ExecuteReboot, MessageBox, _("Install or remove finished.") +" "+_("Do you want to reboot your Settop Box?"), MessageBox.TYPE_YESNO)
		else:
			self.selectedFiles = []
			self.restartRequired = False
		...
		if self.cmdlist is not None:
			for entry in self.cmdlist:
				...
				if cmd == 0:
					...
					info = _("Settop Box software because updates are available.")
...
class PluginDetails(Screen, DreamInfoHandler):
	...
	def runUpgradeFinished(self):
		self.reloadPluginlist()
		if plugins.restartRequired or self.restartRequired:
			self.session.openWithCallback(self.UpgradeReboot, MessageBox, _("Installation finished.") +" "+_("Do you want to reboot your Settop Box?"), MessageBox.TYPE_YESNO)
		else:
			self.close(True)
	...
	def exit(self):
		if not self.ipkg.isRunning():
			if self.packages != 0 and self.error == 0:
				self.session.openWithCallback(self.exitAnswer, MessageBox, _("Upgrade finished.") +" "+_("Do you want to reboot your Settop Box?"))
			else:
				self.close()
		else:
...
class PacketManager(Screen, NumericalTextInput):
	...
	def runRemoveFinished(self):
		self.session.openWithCallback(self.RemoveReboot, MessageBox, _("Remove finished.") +" "+_("Do you want to reboot your Settop Box?"), MessageBox.TYPE_YESNO)
	...
	def runUpgradeFinished(self):
		self.session.openWithCallback(self.UpgradeReboot, MessageBox, _("Upgrade finished.") +" "+_("Do you want to reboot your Settop Box?"), MessageBox.TYPE_YESNO)
